...
var total=0;
var colors=['#f5a045','#4895fa'];
//var colors=['#fd8d3c','steelblue'];
d3.json("data.json", function(error, data) {
    
  
    total= data[region]['cases']
    const maindata = [];
    var pie_data=[];
    Object.keys(data[region]['gender']).map((key)=>{
        pie_data.push(data[region]['gender'][key]/total)*100;
        maindata.push({value:Math.round((data[region]['gender'][key]/total)*100)})
    })
console.log('pie',maindata);
    ...
    var arcs = vis.selectAll("g.slice")     //this selects all <g> elements with class slice (there aren't any yet)
        ...
        arcs.append("svg:text")                                     //add a label to each slice
            ...
            var pieLegend = d3.select("#pie_chart")
            .append("div")
            .attr("id", "pie-legend");
            Object.keys(data[region]['gender']).map((key,i)=>{
...
pieLegend.append("div")
    .html("<strong>Cases by Gender</strong>")
    .style("color", "#008080")
